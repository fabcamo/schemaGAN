...
from layers_functions.generate_rf import rf_generator
...
seed = 20170519         # seed
############################################################################################################
no_realizations = 1    # number of realizations to generate
...
for counter, seed in enumerate(range(seed, seed + no_realizations, 1)):
    ...
    srf_sand = rf_generator(0.3, 1.5, aniso_x, aniso_z, ndim, seed + 1)
    srf_clay = rf_generator(0.3, 2.1, aniso_x, aniso_z, ndim, seed + 2)
    srf_silt = rf_generator(0.5, 3.2, aniso_x, aniso_z, ndim, seed + 3)
    ...
    for i, surfaces in enumerate(zip(surfaces, surfaces[1:])):
        # unpack the consecutive surfaces into two variables top and bot
        top_surf, bot_surf = surfaces
        # concatenate the coordinates over a single list
        poly_points = list(top_surf) + list(bot_surf)


        # using Delunay triangulation, check if the coordinates are inside or outside the polygon
