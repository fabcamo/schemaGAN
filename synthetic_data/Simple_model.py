...
#y_max = 256             # width (y) of the model
z_max = 64              # depth (z) of the model
# Model coordinates
x_coord = np.linspace(1, x_max, x_max, dtype=int)       # array of x coordinates
#y_coord = np.linspace(1, y_max, y_max, dtype=int)       # array of y coordinates
z_coord = np.linspace(1, z_max, z_max, dtype=int)       # array of z coordinates
xs, zs = np.meshgrid(x_coord, z_coord, indexing="ij")   # 2D mesh of coordinates x,z
#xs, ys, zs = np.meshgrid(x_coord, y_coord, z_coord, indexing="ij")     # 3D mesh of coordinates x,y, z
...
seed = 20170519         # seed
############################################################################################################
no_realizations = 50    # number of realizations to generate
...
for counter, seed in enumerate(range(seed, seed + no_realizations, 1)):
    ...
    plt.imshow(df_pivot, aspect="equal")    # plot the pivoted dataframe as an image
    #plt.colorbar()
