...
def generate_real_samples(dataset, no_samples):
    ...
    y = ones((no_samples, 1))  ##Label=1 indicating they are real
    return X, y
...
def generate_noise_vectors(latent_dim, no_samples):
    ...
    x_input = randn(latent_dim * no_samples)
    # reshape into a batch of inputs for the network
    x_input = x_input.reshape(no_samples, latent_dim)
    return x_input
...
def generate_fake_samples(generator, latent_dim, no_samples):
    ...
    x_input = generate_noise_vectors(latent_dim, no_samples)
    # predict using generator to generate fake samples.
    X = generator.predict(x_input)
    # Class labels will be 0 as these samples are fake.
    y = zeros((no_samples, 1))  # Label=0 indicating they are fake
    return X, y
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=128):
    ...
    for i in range(n_epochs):
        ...
        for j in range(bat_per_epo):
            ...
            X_gan = generate_noise_vectors(latent_dim, n_batch)
