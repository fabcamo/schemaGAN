...
def plot_images_error_two_cols(src_img, gen_img, tar_img):
    """
    Plot the source image, generated image, target image, and their absolute error.
    ...
    # Stack all the images
    images = np.vstack((tar_img, src_img, gen_img, np.abs(gen_img - tar_img)))

    # Set plot titles
    titles = ['Original Schematization', 'CPT-like input', 'SchemaGAN Generation', f'Mean absolute error: {mae:.2f}']
    # Set the cbar range
    ranges_vmin_vmax = [[1, 4.5], [0, 4.5], [1, 4.5],  [0, 1]]
    # Set the cbar titles
    cbar_titles = ['IC', 'IC', 'IC', 'IC error']
