...
def compute_errors(original, gan, nn, idw, krig, natnei):
    ...
    with open('results_comparison_MAE.csv', 'w', newline='') as csvfile:
        ...
        for i in range(len(original)):
            writer.writerow([i, mae_gan_list[i], mae_nn_list[i],
                             mae_idw_list[i], mae_krig_list[i], mae_natnei_list[i]])

        # Add another row in the CSV with the mean of each MAE list
        writer.writerow(['Mean', mae_means[0], mae_means[1], mae_means[2], mae_means[3], mae_means[4]])
    ...
    return mae_gan_list, mae_nn_list, mae_idw_list, mae_krig_list, mae_natnei_list, mae_means
